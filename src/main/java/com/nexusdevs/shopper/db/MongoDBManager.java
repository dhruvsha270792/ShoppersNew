...
public class MongoDBManager {
	...
	public boolean setField(String collectionName, Map<String, Object> queryParams, Map<String, Object> fieldValueMap, boolean upsert, boolean multi) {
		...
		try {
			if (multi) {
				collection.updateMany(new Document(queryParams), new Document("$set", new Document(fieldValueMap)),
						new UpdateOptions().upsert(upsert));
			} else {
				collection.updateOne(new Document(queryParams), new Document("$set", new Document(fieldValueMap)),
						new UpdateOptions().upsert(upsert));
			}
			return true;
		} catch (Exception e) {
